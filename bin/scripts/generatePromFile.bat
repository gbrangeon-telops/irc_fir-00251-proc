...
%x_xilperl% %scriptsDir%\setSRecordS0.pl -i %binDir%\tempS3_%fpgaSize%.srec -t %baseName% -o tempS0_%fpgaSize%.srec

rem Generate PROM file
rem %x_promgen% -w -p mcs -spi -c FF -o "%binDir%\prom\%baseName%_%fpgaSize%.mcs" -s 16384 -u 0 %binDir%\download_%fpgaSize%.bit -data_file up 670000 "%releaseFile%" -data_file up 680000 "tempS0_%fpgaSize%.srec" > %binDir%\_promgen_%fpgaSize%.log

IF [%fpgaSize%]==[160] (
%x_promgen% -w -p mcs -spi -c FF -o "%binDir%\prom\%baseName%_%fpgaSize%.mcs" -s 16384 -u 0 %binDir%\download_%fpgaSize%.bit -data_file up 670000 "%releaseFile%" -data_file up 680000 "tempS0_%fpgaSize%.srec" > %binDir%\_promgen_%fpgaSize%.log
 ) ELSE ( 
%x_promgen% -w -p mcs -spi -c FF -o "%binDir%\prom\%baseName%_%fpgaSize%.mcs" -s 16384 -u 0 %binDir%\download_%fpgaSize%.bit -data_file up AF0000 "%releaseFile%" -data_file up B00000 "tempS0_%fpgaSize%.srec" > %binDir%\_promgen_%fpgaSize%.log
 )
if errorlevel 1 (
