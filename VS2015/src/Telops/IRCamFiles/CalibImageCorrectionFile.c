 ...
 * $Id: CalibImageCorrectionFile.c 21422 2018-02-09 04:11:02Z odionne $
 * $URL: http://einstein/svn/firmware/FIR-00251-Proc/trunk/src/IRCamFiles/CalibImageCorrectionFile.c $
 *
 ...
 */
uint32_t CalibImageCorrection_ParseImageCorrectionFileHeader( uint8_t *buffer,  uint32_t bufferLen, fileInfo_t *fileInfo, CalibImageCorrection_ImageCorrectionFileHeader_t *hdr)
{
   ...
   if ((FI_GetFileInfo((const char*)buffer, &fi) != IRC_SUCCESS) || ((fi.type != FT_TSAC) && (fi.type != FT_TSIC)))
   {
      return 0;
   }
   ...
   switch (fi.version.major)
   {
       case 1:
          headerSize = CALIBACTUALIZATION_ACTUALIZATIONFILEHEADER_SIZE_V1;
            if (CalibActualization_ParseActualizationFileHeader_v1(buffer, headerSize, &hdr_v1) == headerSize)
            {
            break;
            }
          return 0;
       ...
       case 2:
          headerSize = CALIBIMAGECORRECTION_IMAGECORRECTIONFILEHEADER_SIZE_V2;
            if (CalibImageCorrection_ParseImageCorrectionFileHeader_v2(buffer, headerSize, hdr) == headerSize)
            {
            break;
            }
          return 0;
 ...
 * @return 0 if an error occurred.
 */
uint32_t CalibImageCorrection_ParseImageCorrectionDataHeader( uint8_t *buffer,  uint32_t bufferLen, fileInfo_t *fileInfo, CalibImageCorrection_ImageCorrectionDataHeader_t *hdr)
{
   ...
   if ((buffer == NULL) || (bufferLen <= MIN_FILE_INFO_SIZE) || (fileInfo == NULL) || ((fileInfo->type != FT_TSAC) && (fileInfo->type != FT_TSIC))  || (hdr == NULL))
   {
      return 0;
   }
   ...
   {
       case 1:
		   offset = CALIBACTUALIZATION_ACTUALIZATIONFILEHEADER_SIZE_V1;
		   headerSize = CALIBACTUALIZATION_ACTUALIZATIONDATAHEADER_SIZE_V1;
            if (CalibActualization_ParseActualizationDataHeader_v1(buffer + offset, headerSize, &hdr_v1) == headerSize)
            {
            break;
            }
          return 0;
       ...
       case 2:
		   offset = CALIBIMAGECORRECTION_IMAGECORRECTIONFILEHEADER_SIZE_V2;
		   headerSize = CALIBIMAGECORRECTION_IMAGECORRECTIONDATAHEADER_SIZE_V2;
            if (CalibImageCorrection_ParseImageCorrectionDataHeader_v2(buffer + offset, headerSize, hdr) == headerSize)
            {
            break;
            }
          return 0;
