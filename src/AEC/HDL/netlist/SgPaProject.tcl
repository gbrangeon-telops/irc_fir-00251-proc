namespace eval ::xilinx::dsp::planaheadworker {
   ...
   proc dsp_get_devicename { } {
      ...
      if { $paramvalueFamily == "kintex8" } {
          set paramvaluePackage "-$paramvaluePackage"
      } elseif { $paramvalueFamily == "virtex8" } {
          set paramvaluePackage "-$paramvaluePackage"
      } 
   ...
   proc dsp_add_project_files {} {
      set projfilesexts "xco ucf xdc mif coe ngc v vhd testbench tcl dat"
...
proc dsp_ip_packager_handler_coe { filelist } {
 ...
 proc dsp_get_xilinx_ip_repository_root {} {
     ...
     set index_file_name ""
     set rdi_data_dirs [split $rdi_data_dir_env ":"]
 ...
 proc dsp_ipp_get_synthesis_file_group {} {
     ...
     set synth_file_group [ipx::get_file_group -quiet $synth_file_group_text [ipx::current_core]]
     if { $synth_file_group == ""} {
 ...
 proc dsp_ipp_get_softwaredrivers_file_group {} {
     ...
     set file_group [ipx::get_file_group -quiet $file_group_text [ipx::current_core]]
     if { $file_group == ""} {
 ...
 proc dsp_ipp_get_datasheet_file_group {} {
     set datasheet_file_group [ipx::get_file_group -quiet xilinx_datasheet [ipx::current_core]]
 ...
 proc dsp_ipp_get_testbench_file_group {} {
     ...
     set tb_file_group [ipx::get_file_group -quiet $tb_file_group_text [ipx::current_core]]
     if { $tb_file_group == "" } {
 ...
 proc dsp_ipp_get_simulation_file_group {} {
     ...
     set sim_file_group [ipx::get_file_group -quiet $sim_file_group_text [ipx::current_core]]
     if { $sim_file_group == ""} {
 ...
 proc dsp_ipp_handler_v { filelist } {
     ...
     foreach ffile $filelist {    
         ...
         if { $top_module_file == [file tail $ffile] } {
             puts "Specifying Ports in the Top Level"
 ...
 proc dsp_ipp_create_ips {} {
  ...
  proc dsp_ipp_infer_bus_interface { ip_core } {
      if {[dsp_ip_packager_get_Auto_Infer_flag] == "1" } {
	      ipx::infer_bus_interface {{xilinx.com:interface:axis:1.0} {xilinx.com:interface:aximm_rtl:1.0} {xilinx.com:signal:data_rtl:1.0}} $ip_core
  ...
  proc dsp_package_for_vivado_ip_integrator {} {
    ...
    set projfilesexts "xci ucf xdc xco dat coe mif ngc vhd v h c htm mdd mtcl mak html"
    ...
    #Collect all the .driver files separately    
    #set datapath [ dsp_get_sysgen_project_file_search_path_list ]
    #set listvarname [ dsp_get_list_var_name "h" ]
    #set $listvarname [ dsp_get_file_name_list_from_pathlist $datapath {.h} ]
    #set listvarname [ dsp_get_list_var_name "c" ]
    #set $listvarname [ dsp_get_file_name_list_from_pathlist $datapath {.c} ]
 ...
 proc dsp_ip_packager_testbench_and_document  { }  {
    ...
    set folder [dsp_ip_packager_get_location]
     ...
     if { [dsp_hdllang_is_vhdl] } {
          if { [ipx::get_file_group -quiet xilinx_vhdlsynthesis $currentCore] == "" } {
              set synth [ipx::get_file_group xilinx_anylanguagesynthesis $currentCore]
              set sim [ipx::get_file_group xilinx_anylanguagebehavioralsimulation $currentCore]
          } else {
	          set synth [ipx::get_file_group xilinx_vhdlsynthesis $currentCore]
              set sim [ipx::get_file_group xilinx_vhdlbehavioralsimulation $currentCore]
          }
          if { [ dsp_has_testbench ] }  {
              set test [ipx::get_file_group xilinx_vhdltestbench $currentCore]
           }
       } else {
          if { [ipx::get_file_group -quiet xilinx_verilogsynthesis $currentCore] == "" } {
              set synth [ipx::get_file_group xilinx_anylanguagesynthesis $currentCore]
              set sim [ipx::get_file_group xilinx_anylanguagebehavioralsimulation $currentCore]
          } else {
              set synth [ipx::get_file_group xilinx_verilogsynthesis $currentCore]
              set sim [ipx::get_file_group xilinx_verilogbehavioralsimulation $currentCore]
          }	 
          if { [ dsp_has_testbench ] }  {
 ...
 proc dsp_ip_packager_add_icon {} {
    ...
    set nam [get_property name $currentCore]
    set ver [get_property version $currentCore]
    set verion [string map {"." "_"} $ver]
    set filename [format "xgui/%s_v%s.tcl" $nam $verion]
    set line0 "\#Definitional proc to organize widgets for parameters.\n"
    set line1 "proc create_gui \{ ipview \} \{\n"
    set line2 "    set_property ip_logo ip_catalog/icon/sysgen_icon_100.png \[ipgui::get_canvasspec -of \$ipview\]\n"
	set line3 "    set Page0 \[ ipgui::add_page \$ipview  -name \"Page 0\" -layout vertical \]\n"
	set line4 "    set Component_Name \[ ipgui::add_param  \$ipview  -parent  \$Page0  -name Component_Name \]\n"
    set line5 "\}"
    set data " $line0 $line1 $line2 $line3 $line4 $line5"
 ...
 proc dsp_apply_bd_automation_for_microblaze {} {
    ...
    set proc_sys_reset_inst proc_sys_reset_1
    ...
    foreach intf $intfs {
        ...
        if { $index != -1 } {
            ...
            if {[get_property MODE $intf] == "SLAVE"} {
                set_property CONFIG.HAS_TLAST [get_property CONFIG.HAS_TLAST [get_bd_intf_pins /$ip_inst/[get_property NAME $intf]] [get_property NAME $intf]
                set_property CONFIG.LAYERED_METADATA [get_property CONFIG.LAYERED_METADATA [get_bd_intf_pins /$ip_inst/[get_property NAME $intf]] [get_property NAME $intf]
    ...
    foreach pin $pins {
        ...
        if { $is_intf_pin == FALSE && $pin_net == "" } {
            ...
            if {[get_property -quiet LEFT $pin] == ""} {
                # A boolean signal
                create_bd_port -dir [get_property -quiet DIR $pin] -type data $pin_name 
            } else {
                create_bd_port -dir [get_property -quiet DIR $pin] -from [get_property -quiet LEFT $pin] -to [get_property -quiet RIGHT $pin] -type data $pin_name 
            }
