...
/**
 * Indicates if tag is dirty. A tag is dirty if first 4 bytes not all 0xFF.
 *
 * Macro has been replaced by a function to avoid having the
 * following warning message:
 *
 * "Type dereferencing type-punned pointer will break strict-aliasing rules [-Wstrict-aliasing]"
 */
u8 TAG_IS_DIRTY(uffs_Tags *tag)
{
   u8 i;
   u8 *p_data = (u8 *) &((tag)->s);
