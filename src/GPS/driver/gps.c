 ...
 */
IRC_Status_t GPS_Init(t_GPS *GPS_Data, uint16_t uartDeviceId, XIntc *intc,
      uint16_t uartIntrId, uint8_t *rxCircBuffer, uint16_t rxCircBufferSize)
{
   IRC_Status_t status;
   ...
   switch (GPS_Data->index)
   {
      ...
      case 3:		// Process the NMEA frame
         ...
         if (strcmp((char *) (GPS_Data->dataArray[0]), "$GPRMC") == 0)
         {
            ...
            if (GPS_Data->showNMEA)
               GPS_INF("GPRMC sentence received");
         }
         else if (strcmp((char *) (GPS_Data->dataArray[0]), "$GPGGA") == 0)
         {
            returnValue = GPS_GPGGA;
            if (GPS_Data->showNMEA)
               GPS_INF("GPGGA sentence received");
         }
         else
         {
            returnValue = GPS_NMEA_OTHERS;
            if (GPS_Data->showNMEA)
               GPS_INF("Other NMEA sentence received");
         }
