...
#define MGLK_XTRA_TRIG_FREQ_MAX_HZ        44     // soit une frequence de 44Hz         
...
#define MODE_READOUT_END_TO_TRIG_START    0x00    // provient du fichier fpa_common_pkg.vhd. Ce mode est choisi car plus simple pour le ScorpioLW
...
#define FPA_ROIC_SCORPIO_LW               0x17   // provient du fichier fpa_common_pkg.vhd.
...
#define MGLK_GPOL_VALUE_MIN               3000     // 3000 mV comme valeur minimale de GPOL
#define MGLK_GPOL_VALUE_MAX               5500     // 5500 mV comme valeur maximale de GPOL
...
void  FPA_SoftwType(const t_FpaIntf *ptrA)
{
   AXI4L_write32(FPA_ROIC_SCORPIO_LW, ptrA->ADD + AW_FPA_ROIC_SW_TYPE);          
...
//      ptrH->T6     = 7.0F + pGCRegs->Width/4.0F;
//      ptrH->T0     = ptrH->T4 + ptrH->T2 + ptrH->T6 + ptrH->T3 + ptrH->T5;
   ...
   {      
      ...
      ptrH->T3     = kk.T3*(float)pGCRegs->Height + kk.T4*((float)pGCRegs->Height-1.0F);
      ptrH->T4     = 1.0F; 
      ptrH->T5     = (50.0E-6F)/ptrH->TMCLK + 4.0F;
      ptrH->T6     = 7.0F + pGCRegs->Width/4.0F + 15.0F + (float)VHD_FVAL_LOW_GEN_DLY_MCLK; // 15.0F = delai du MGLK
      ptrH->T0     = ptrH->T4 + ptrH->T2 + ptrH->T6 + ptrH->T3 + ptrH->T5;
   }
...
{    
   ...
   FPA_FillCmdCharTable(0x100, 0x03, 1, &Cmd, &CharIndex);        // adresse 0x100 : double base mode, 4 taps, normal consumption, no inversion 
...
{    
   ...
   CmdByte = 0x03;          // valeur par defaut pour ne pas avoir d'inversion et rouler tout le temps en mode windowing pour éviter offset d'image.
   if (FPA_FLIP_UD == 1)
      CmdByte &= 0xFE;
   if (FPA_FLIP_LR == 1)
      CmdByte &= 0xFD;
   //if ((ptrA->proxy_xsize != (uint32_t)FPA_WIDTH_MAX) || (ptrA->proxy_ysize != (uint32_t)FPA_HEIGHT_MAX))
   //CmdByte &= 0xF3;
   //else
   //   CmdByte |= 0x0C;
