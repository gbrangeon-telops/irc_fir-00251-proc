...
#define ELCORR_CONT_MODE_OFFSETX_MIN      640 // 128 // FPA_WIDTH_MAX permet de ne plus être en mode continuel de calcul du gain. ENO 27 avril 2019: Pour le ISC0804, ne plus le faire
...
uint8_t speedup_lsydel         = 0;      // les speed_up n'ont que deux valeurs : 0 ou 1
...
//   ptrA->permit_lsydel_clk_rate_beyond_2x = (uint32_t)gFpaDebugRegG; 
   ...
   // adc clk source phase
   if (init_done == 0){         
      ptrA->adc_clk_source_phase = 348480; //179200;
      if ((Stat.flex_flegx_detect_process_done == 1) && (Stat.flex_flegx_present == 1) && (flegx_present == 0))  // cas du LN2
         ptrA->adc_clk_source_phase = 230400; 
   }   
   ...
   if ((gFpaDebugRegB != (int32_t)ptrA->roic_dbg_reg) && (init_done == 1) && (roic_dbg_reg_unlocked == 1))  // une fois debloqué, le registre peut etre changé 
  ...
  if (elcorr_reg == 19){            // pixeldata avec correction du gain et offset electroniques en mode continuel puis ref dans l'image
   ...
   // reset fpa module (suite à l'envoi des registres c et d
   ...
   // fin de l'initialisation. Mis ici pour supporter aussi le LN2 
   if (Stat.flex_flegx_detect_process_done == 1)
      init_done = 1; 
}
...
int16_t FPA_GetTemperature(const t_FpaIntf *ptrA)
{
   ...
   diode_voltage = (float)Stat.fpa_temp_raw * ((float)FPA_TEMP_READER_FULL_SCALE_mV/1000.0F) / (powf(2.0F, FPA_TEMP_READER_ADC_DATA_RES) * (float)FPA_TEMP_READER_GAIN);
   ...
   if ((flashSettings.FPATemperatureConversionCoef4 == 0) && (flashSettings.FPATemperatureConversionCoef3 == 0) &&
      ...
      if ((Stat.flex_flegx_detect_process_done == 1) && (Stat.flex_flegx_present == 1) && (flegx_present == 0)){
