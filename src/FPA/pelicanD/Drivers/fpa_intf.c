...
#define T0_CORR                           39.99E-6F // [s],  Nécessaire pour maintenir des specs équivalentes entre IWR et ITR (voir redmine 14065 pour justifications).
...
void FPA_Init(t_FpaStatus *Stat, t_FpaIntf *ptrA, gcRegistersData_t *pGCRegs)
{
   ...
   FPA_EnableSerdesInit(ptrA, true);                                        // Always true for PelicanD (only used by BB1280)
   FPA_EnableFailureResponseManagement(ptrA, true);                         // Always true for PelicanD (only used by BB1280)
   FPA_EnableSerialExposureTimeCMD(ptrA, true);                             // Always false for PelicanD (only used by BB1280)
