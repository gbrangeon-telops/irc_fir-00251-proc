...
// Parametres de la commande serielle du ScorpioLW
#define MGLK_LONGEST_CMD_CHAR_NUM         128     // longueur en bytes de la plus longue commande serielle du ScorpioLW )(en realité , moins que ça mais juste pour reserver de la memoire)

// Mode d'operation choisi pour le contrôleur de trig 
#define MODE_READOUT_END_TO_TRIG_START    0x00    // provient du fichier fpa_common_pkg.vhd. Ce mode est choisi car plus simple pour le MARS
#define MODE_INT_END_TO_TRIG_START        0x02    // provient du fichier fpa_common_pkg.vhd

// ScorpioLW integration modes definies par Mglk  
...
#define FPA_ROIC_MARS                     0x10   // provient du fichier fpa_common_pkg.vhd.
...
#define MGLK_X1MIN_LSB_ADD                0x1D0     // adresse d'ecriture du LSB de X1MIN pour megalink scorpioLW
#define MGLK_Y1MIN_LSB_ADD                0x1D2     // adresse d'ecriture du LSB de Y1MIN pour megalink scorpioLW
#define MGLK_X1MAX_LSB_ADD                0x1D4     // adresse d'ecriture du LSB de X1MAX pour megalink scorpioLW
#define MGLK_Y1MAX_LSB_ADD                0x1D6     // adresse d'ecriture du LSB de Y1MAX pour megalink scorpioLW
//#define MGLK_GPOL_VALUE                   0x03CF    // valeur par defaut de GPOL en millivolts. Sur approbation de PTR, on reconduit GPOL = 0.975 V de la carte 180.

#define MGLK_GPOL_VALUE_MIN               500        // 500 mV comme valeur minimale de GPOL
#define MGLK_GPOL_VALUE_MAX               1000       // 1000 mV comme valeur maximale de GPOL

#define VHD_CFG_ARBITER_DLY_MIN           50         // cette durée minimale permettra à la machine à état VHD de saisir la commande
#define MGLK_INT_SIGNAL_IS_EXTERNAL       1
#define MGLK_MASTER_CLOCK_IS_EXTERNAL     1
...
void FPA_SendConfigGC(t_FpaIntf *ptrA, const gcRegistersData_t *pGCRegs)
{
   ...
   // gain
   ptrA->proxy_gain = MGLK_GAIN_0;
   if (pGCRegs->SensorWellDepth == SWD_HighGain)
   ptrA->proxy_gain = MGLK_GAIN_1;
   ...
   // calculé specialement pour le scorpioLW
...
void  FPA_SoftwType(const t_FpaIntf *ptrA)
{
   AXI4L_write32(FPA_ROIC_MARS, ptrA->ADD + AW_FPA_ROIC_SW_TYPE);          
...
void FPA_Fig2SpecificParams(Proxy_Fig2Param_t *ptrH, float exposureTime_usec, const gcRegistersData_t *pGCRegs)
{
   ...
   ptrH->T4     = 1.0F;
   ptrH->T5     = 0.0F;
   ptrH->T6     = 19.0F + 15.0F + (float)VHD_FVAL_LOW_GEN_DLY_MCLK;   // 15.0 MCLK -> Megalink et 19.0F pour Mars
   ptrH->T0     = ptrH->T4 + ptrH->T2 + MAX(ptrH->T6 + ptrH->T3 + ptrH->T5, 0.0F);  // en coups d'horloge MASTERCLK. Le 250 est porté à disparaitre avec la carte ADC Yaris. C'est une limitation de la carte 77.
...
void FPA_Fig4SpecificParams(Proxy_Fig4Param_t *ptrK, const gcRegistersData_t *pGCRegs)
{
   ...
   // se reporter au document D:\Telops\FIR-00251-Proc\src\FPA\Megalink\Doc\Chronogrammes.doc
   // utliser en mode diag
   ptrK->T1  = 0.0F;
   ptrK->T2  = 0.0F;
   ptrK->T3  = (float)pGCRegs->Width/(float)FPA_NUMTAPS;
   ptrK->T4  = 16.0F/4.0F;   // selon la doc, le megalink introduit un delai de 16 coups d'horloges XLINK entre les lignes. Comme FreqXLINK vaut 4x FreqMCLK... 
   ptrK->T5  = 0.0F;
   ptrK->T6  = 0.0F;       //
}
...
void FPA_SendOperational_SerialCmd(const t_FpaIntf *ptrA)
{
   ...
   CmdByte = 0x00;          // valeur par defaut à 0x00 tout le temps en mode windowing pour éviter offset d'image
   ...
   // adresse 0x1A0 : windowing mode continuelle et Gain 
   if (ptrA->proxy_gain == MGLK_GAIN_0)
      CmdByte |= 0x40;
...
{    
   ...
   FPA_FillCmdCharTable(0x141, 0x02, 1, &Cmd, &CharIndex);        // adresse 0x141 : video2 on channelX, Video2 in 2nd position in clink frame
   FPA_FillCmdCharTable(0x142, 0x04, 1, &Cmd, &CharIndex);        // adresse 0x142 : video3 on channelX, Video3 in 3rd position in clink frame
